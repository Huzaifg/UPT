...
URLS = {
    ...
    "column2d": "/2D_MPM_2304.zip"  # Hack to make it get the right path
}
...
class LagrangianDataset(DatasetBase):
    def __init__(
        ...
        assert split in ["train", "valid",
                         "test"], f"Split {split} not available."
        ...
        assert graph_mode in [
            'knn', 'radius_graph', 'radius_graph_with_supernodes'], f"graph_mode {graph_mode} not available."
        ...
        if local_root is None:
            ...
            # remove localrank0 from local_root
            local_root = local_root.parent
            self.source_root = local_root / ds_name
            # Check if dataset needs to be downloaded
            if not os.path.isdir(self.source_root):
                self.logger.info(f"downloading '{ds_name}'")
                self.download(name, local_root)
    ...
    def get_window(self, idx: int, ctx=None, downsample=False):
        ...
        if self.num_points_range:
            if self.num_points_range[0] == self.num_points_range[1]:
                ...
                num_points_range = torch.rand(
                    size=(1,), generator=None).item() * (ub - lb) + lb
                end = int(num_points_range)
            # uniform sampling
            # perm = torch.randperm(self.n_particles, generator=None)[:end]
            perm = torch.randperm(ctx["window"][0].shape[1], generator=None)[
                :min(end, ctx["window"][0].shape[1])]
            ctx["perm"] = perm
    ...
    def get_velocities(self, positions):
        ...
        if self.box is not None:
            ...
            velocities = (velocities + self.box *
                          0.5) % self.box - 0.5 * self.box
        # Normalization
    ...
    def get_accelerations(self, positions):
        ...
        if self.box is not None:
            # Calculation of PBC is done like in jax_md.space.periodic
            next_velocities = (next_velocities + self.box *
                               0.5) % self.box - 0.5 * self.box
            current_velocities = (current_velocities +
                                  self.box * 0.5) % self.box - 0.5 * self.box
    ...
    def getitem_x(self, idx, ctx=None):
        ...
        input_features = self.get_velocities(
            positions[:self.n_input_timesteps, :, :])
    ...
    def getitem_edge_index(self, idx, ctx=None, downsample=True):
        ...
        if self.graph_mode == 'knn':
            knn = KNNGraph(k=self.knn_graph_k, loop=True,
            ...
            perm_supernodes = torch.randperm(
                current_input_position.shape[0], generator=generator)[:self.n_supernodes]
    ...
    def getitem_edge_index_target(self, idx, ctx=None, downsample=True):
        ...
        if self.graph_mode == 'knn':
            ...
            # select supernodes
            generator = self._get_generator(idx)
            perm_supernodes = torch.randperm(target_position.shape[0], generator=generator)[
                :self.n_supernodes]
    ...
    def getitem_edge_features(self, idx, ctx=None, downsample=True):
        edge_index = self.getitem_edge_index_target(
            idx, ctx, downsample=downsample)
        positions, _ = self.getter(idx, ctx, downsample=True)
        target_position = positions[-1, :, :]
        relative_displacement = target_position[edge_index[:, 0]
                                                ] - target_position[edge_index[:, 1]]
        distance = ((target_position[edge_index[:, 0]] -
                    target_position[edge_index[:, 1]])).norm(dim=-1)
    ...
    def getitem_target_acc(self, idx, ctx=None):
        positions, _ = self.getter(idx, ctx, downsample=False)
        target_features = self.get_accelerations(
            positions[-3:, :, :]).squeeze()
        return target_features
